...
| CM-6 | Configuration Settings | `infrastructure/kyverno/policies/`, `infrastructure/compliance/host-posture.py`, `kube-bench` | [`docs/evidence/samples/host-posture-20260801015522.json`](evidence/samples/host-posture-20260801015522.json) — real host security posture pulled live from appserv (LSM, FIPS, ASLR, SSH config, patching, host firewall), reported honestly including "not applicable" results; `kube-bench` baseline still Phase 14 | generated (partial) |
