...
| DORA Art. 24–27 | Resilience testing | `drills/` (four scenarios: `ns-restore`, `node-kill`, `pvc-corruption-restore`, `network-partition`) | [`docs/evidence/samples/scenario-breadth-20260730021814.txt`](evidence/samples/scenario-breadth-20260730021814.txt) — real, live drill runs across three distinct fault types beyond the original single scenario; CIS benchmark output (kube-bench) still not yet generated | generated (partial — drill breadth only, kube-bench still Phase 14) |
