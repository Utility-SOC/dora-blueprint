...
- Regulatory article references follow `docs/00-scope.md` §0.2: DORA is lex specialis to NIS2 for
  Meridian Pay's ICT risk management, incident reporting, resilience testing, and third-party
  risk domains, so the DORA rows are the binding obligation and the NIS2 rows exist to make the
  divergence in reporting-clock start events (§0.4) visible, not because both regimes apply
  independently.
