...
|---|---|---|---|---|
| DORA Art. 9 | Protection & prevention | `infrastructure/kyverno/policies/`, `infrastructure/cilium/` | [`docs/evidence/samples/kyverno-admission-20260728202845.txt`](evidence/samples/kyverno-admission-20260728202845.txt) — real admission denial of a privileged pod; [`docs/evidence/samples/cilium-network-segmentation-20260729015300.txt`](evidence/samples/cilium-network-segmentation-20260729015300.txt) — default-deny blocking unwanted egress while explicit allows and cross-namespace DNS keep working, across all 8 segmented namespaces; [`docs/evidence/samples/scenario-breadth-20260730021814.txt`](evidence/samples/scenario-breadth-20260730021814.txt) — the `network-partition` drill, a repeatable, re-runnable validation (not a one-time snapshot) that a real probe genuinely fails during a live Cilium deny and genuinely succeeds once it's removed | generated |
| DORA Art. 10 | Detection | `apps/grafana.yaml` (Alerting rules on the kube-audit Loki stream and, as of Phase 13, on Tetragon's eBPF process-exec stream), `drills/templates/ns-restore-workflowtemplate.yaml`, `drills/templates/credential-compromise-workflowtemplate.yaml` | [`docs/evidence/samples/detection-latency-20260729135024.txt`](evidence/samples/detection-latency-20260729135024.txt), [`docs/evidence/samples/ns-restore-20260729135024.json`](evidence/samples/ns-restore-20260729135024.json) — a real Grafana Alerting rule independently observing the fault via the audit log, a real non-null `t_detect` captured from Grafana's own live Alerting API; [`docs/evidence/samples/credential-compromise-20260730044858.txt`](evidence/samples/credential-compromise-20260730044858.txt) — a real shell spawn genuinely observed by Tetragon's eBPF instrumentation, detected in 11s, an independent detection surface (endpoint/process, not just the audit log) | generated |
...
| DORA Art. 24–27 | Resilience testing | `drills/` (five scenarios: `ns-restore`, `node-kill`, `pvc-corruption-restore`, `network-partition`, `credential-compromise`) | [`docs/evidence/samples/scenario-breadth-20260730021814.txt`](evidence/samples/scenario-breadth-20260730021814.txt) — real, live drill runs across three distinct fault types beyond the original single scenario; [`docs/evidence/samples/credential-compromise-20260730044858.txt`](evidence/samples/credential-compromise-20260730044858.txt) — a fourth distinct fault type (runtime/endpoint), validating the detection path rather than break/recover; CIS benchmark output (kube-bench) still not yet generated | generated (partial — drill breadth only, kube-bench still Phase 14) |
...
| ISO A.8.16 | Monitoring | `apps/grafana.yaml` (Alerting rule on the kube-audit Loki stream), `infrastructure/kyverno/policies/require-pod-security-restricted.yaml` + `apps/tetragon.yaml` (Phase 13, eBPF process-exec monitoring) | [`docs/evidence/samples/detection-latency-20260729135024.txt`](evidence/samples/detection-latency-20260729135024.txt) — real detection latency (`t_detect` − `t_inject`) = 6s, plus two earlier real detection failures left honest rather than papered over; [`docs/evidence/samples/credential-compromise-20260730044858.txt`](evidence/samples/credential-compromise-20260730044858.txt) — real endpoint-level process monitoring, a genuine `execve` observed and alerted on in 11s | generated |
...
| ISO A.8.20 | Networks security | `infrastructure/cilium/`, `drills/templates/network-partition-workflowtemplate.yaml` | [`docs/evidence/samples/scenario-breadth-20260730021814.txt`](evidence/samples/scenario-breadth-20260730021814.txt) — a real, repeatable drill proving Cilium enforcement in both directions (deny and recovery), not just a one-time verification snapshot | generated |
